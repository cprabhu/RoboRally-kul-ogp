...
public class Robot extends Element implements EnergyElement {
     ...
     * Hit this Robot.
     * 
     * @effect This Robot it's maximum energy will decrease with 4000 Ws.
     *      | new.getMaxEnergy().getAmountOfEnergy() =
     *      |   getMaxEnergy().getAmountOfEnergy() - 4000
     * @effect This Robot's energy may not exceed it's maximum energy.
     *      | If (getAmountOfEnergy() > new.getMaxEnergy())
     *      |   new.getAmountOfEnergy() = new.getMaxEnergy().getAmountOfEnergy()
     * @effect If this Robot's maximum energy reaches 0 it will terminate itself.
     *      | If (getMaxEnergy().getAmountOfEnergy() <= 0)
     *      |   this.terminate()
     */
    public void hit() {
    ...
    /**
     * Add a set of items to this Robot's possessions while keeping them ordered.
     * 
     * @param itemsToAdd
     *      The set of items to add.
     * @post The items are added to this Robot's possessions if it has not been
     *      terminated and the item isn't null.
     *      | if (!isTerminated())
     *      |   for itemToAdd in itemsToAdd
     *      |       if (itemToAdd != null)
     *      |           new.getPossessions().contains(itemToAdd)
     * @post This Robot's possessions are sorted by weight from heavy to light.
     *      | 0 <= i <= j < new.getPossessions().size()
     *      | new.getPossessions().get(i).compareTo(
     *      |   new.getPossessions().get(i + j)) >= 0
     */
