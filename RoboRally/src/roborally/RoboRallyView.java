...
public class RoboRallyView<Board, Robot, Wall, Battery> extends JPanel {
    ...
    public RoboRallyView(final RoboRally<Board, Robot, Wall, Battery> roboRally) {
        ...
        try {
            robotImage = ImageIO.read(getClass().getClassLoader().getResource(
                    "res/robot1.jpg"));
            wallImage = ImageIO.read(getClass().getClassLoader().getResource(
                    "res/wall.jpg"));
            batteryImage = ImageIO.read(getClass().getClassLoader()
                    .getResource("res/battery.png"));
        } catch (IOException e) {
        ...
        this.addMouseMotionListener(new MouseMotionListener() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                IFacade<Board, Robot, Wall, Battery> facade = roboRally
                        .getFacade();
                for (Robot robot : facade.getRobots(roboRally.getBoard())) {
                    ...
                    if (xr == x && yr == y) {
                        roboRally.setStatus(roboRally.getRobotName(robot)
                                + ": " + robot.toString());
                        return;
                    }
                }
    ...
    public void paintComponent(Graphics g) {
        ...
        for (Battery battery : facade.getBatteries(roboRally.getBoard())) {
            ...
            if (x < Integer.MIN_VALUE + 2 * TILE_SIZE
                    || x > Integer.MAX_VALUE - 2 * TILE_SIZE
            ...
            // draw name
            g.drawString(roboRally.getBatteryName(battery), tileXRoot + 2,
                    tileYRoot + g.getFontMetrics().getAscent() - 2);
        }
        for (Robot robot : facade.getRobots(roboRally.getBoard())) {
            ...
            // draw name
            g.drawString(roboRally.getRobotName(robot), tileXRoot + 2,
                    tileYRoot + g.getFontMetrics().getAscent() - 2);
            // draw orientation
            int[] xPoints;
            int[] yPoints;
            ...
            if (roboRally.getFacade().getOrientation(robot) == 0) {
                xPoints = new int[] { orientationXRoot - 6, orientationXRoot,
                        orientationXRoot + 6 };
                yPoints = new int[] { orientationYRoot, orientationYRoot - 6,
                        orientationYRoot };
            } else if (roboRally.getFacade().getOrientation(robot) == 1) {
                xPoints = new int[] { orientationXRoot, orientationXRoot + 6,
                        orientationXRoot };
                yPoints = new int[] { orientationYRoot - 6, orientationYRoot,
                        orientationYRoot + 6 };
